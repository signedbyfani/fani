...
function getAdapter({ edgeMiddleware, middlewareSecret, skewProtection, buildOutput, }) {
    return {
        name: PACKAGE_NAME,
...
export default function vercelAdapter({ webAnalytics, includeFiles: _includeFiles = [], excludeFiles: _excludeFiles = [], imageService, imagesConfig, devImageService = 'sharp', edgeMiddleware = false, maxDuration, isr = false, skewProtection = false, } = {}) {
    if (maxDuration) {
    ...
    return {
        ...
        hooks: {
            ...
            'astro:build:ssr': async ({ entryPoints, middlewareEntryPoint }) => {
                _entryPoints = new Map(Array.from(entryPoints)
            ...
            'astro:build:done': async ({ logger }) => {
                ...
                if (_buildOutput === 'server') {
                    ...
                    const builder = new VercelBuilder(_config, excludeFiles, includeFiles, logger, outDir, maxDuration);
                    ...
                    if (_entryPoints.size) {
                        ...
                        if (isr) {
                            ...
                            for (const route of routes) {
                                ...
                                const dest = src.startsWith('^\\/_image') || src.startsWith('^\\/_server-islands')
                                    ? NODE_PATH
                                    : ISR_PATH;
...
class VercelBuilder {
    ...
    async buildISRFolder(entry, functionName, isr, root) {
        ...
        const prerenderConfig = new URL(`./functions/${functionName}.prerender-config.json`, this.outDir);
